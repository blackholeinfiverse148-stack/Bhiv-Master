...
import {
  ...
  DollarSign, BarChart2, Target, ShieldCheck, ArrowRight, Search,
} from "lucide-react";
...
// ─────────────────────────────────────────────────────────────
var DASHBOARDS = [
  { id: "shakti", label: "SHAKTI Master", icon: Layers, color: "#6366F1" },
  { id: "executive", label: "Executive Core", icon: LayoutGrid, color: DS.accent.primary },
  { id: "operations", label: "Operations Command", icon: Activity, color: DS.status.warning },
  { id: "engineering", label: "Engineering Core", icon: Cpu, color: DS.chart[2] },
  { id: "soc", label: "SOC Security Gateway", icon: ShieldCheck, color: DS.status.critical },
  { id: "finance", label: "Finance Ledger", icon: DollarSign, color: DS.status.healthy },
  { id: "analytics", label: "Analytics Node", icon: BarChart3, color: DS.chart[4] },
...
function Sidebar({ activeDash, setActiveDash }) {
  ...
  var toggleSection = function (secKey) {
    setExpanded(function (prev) {
      var next = Object.assign({}, prev);
      next[secKey] = !prev[secKey];
      return next;
    });
  };

  var filteredGroups = DASHBOARDS_GROUPED.map(function (g) {
    var items = g.items.filter(function (it) {
      return it.label.toLowerCase().indexOf(search.toLowerCase()) !== -1;
    });
    return { title: g.title, key: g.key, items: items };
  }).filter(function (g) {
    return g.items.length > 0;
  });

  var isExpanded = function (secKey) {
    if (search.trim() !== "") return true;
    return expanded[secKey];
  };
        ...
        {filteredGroups.map(function (g) {
          ...
          return (
            <div key={g.key} style={{ marginBottom: 12 }}>
              ...
              >
                <span>{g.title}</span>
                {exp ? <ChevronDown size={11} /> : <ChevronRight size={11} />}
              </div>
              {exp && (
                  ...
                  {g.items.map(function (d) {
                    ...
                    return (
                      <button key={d.id} onClick={function () { setActiveDash(d.id); }}
                        style={{ width: "100%", display: "flex", alignItems: "center", gap: 9, padding: "6px 8px", marginBottom: 1, borderRadius: DS.radius.md, border: "none", background: isA ? (theme === "dark" ? "#1A2436" : "#E8F0FE") : "transparent", color: isA ? DS.accent.primary : t.textSub, fontWeight: isA ? 600 : 500, fontSize: 12, cursor: "pointer", fontFamily: DS.font.body, textAlign: "left" }}
                        ...
                        <d.icon size={13} strokeWidth={2.2} color={isA ? DS.accent.primary : d.color + "99"} />
...
function ShaktiMasterDashboard() {
  ...
  return (
    <div style={{ padding: "14px 18px 30px", animation: "fadeIn .25s ease" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 14 }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid " + t.border, textAlign: "left" }}>
                  <th style={{ padding: "6px 8px", color: t.textMuted, fontWeight: 600 }}>NODE</th>
                  <th style={{ padding: "6px 8px", color: t.textMuted, fontWeight: 600 }}>TYPE</th>
                  <th style={{ padding: "6px 8px", color: t.textMuted, fontWeight: 600 }}>LATENCY</th>
                  <th style={{ padding: "6px 8px", color: t.textMuted, fontWeight: 600 }}>INTEGRITY HASH</th>
                  <th style={{ padding: "6px 8px", color: t.textMuted, fontWeight: 600 }}>VERDICT</th>
                </tr>
              </thead>
              <tbody>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <Card t={t} p={12}>
            <SectionHeader t={t} title="Federation Engine Logs" sub="Real-time handshake and state synchronization logs" />
            <div style={{ background: t.bgElevated, border: "1px solid " + t.border, borderRadius: 6, padding: 8, marginTop: 8, fontFamily: DS.font.mono, fontSize: 10, height: 160, overflowY: "auto", display: "flex", flexDirection: "column", gap: 6 }}>
              {log.map(function (l, i) {
                return (
                  <div key={i} style={{ color: t.textSub, lineHeight: 1.4 }}>
                    <span style={{ color: t.textMuted, marginRight: 6 }}>[{l.time}]</span>
                    {l.msg}
                  </div>
                );
              })}
          ...
          <Card t={t} p={12}>
            <SectionHeader t={t} title="Federated Verification Control" sub="Verify telemetry trace signatures against the Bucket provenance store" />
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 10 }}>
              <div style={{ background: t.surfaceHover, border: "1px solid " + t.border, borderRadius: 6, padding: "8px 10px", fontSize: 11, color: t.textSub }}>
                <div style={{ fontWeight: 600, color: t.text, marginBottom: 2 }}>Contract Status: ACTIVE</div>
                All services are adhering to the telemetry schema mapped for the Shakti federation.
              </div>
              <Btn variant="ghost" t={t} onClick={function () {
                if (addAudit) addAudit({ label: "Verify Federation Hashes", target: "KARMA", status: "success" });
                if (addNotif) addNotif({ text: "Integrity hashes match primary DB record", severity: "info" });
              }} style={{ justifyContent: "center" }}>
                <ShieldCheck size={12} /> Verify All Signatures
              </Btn>
            </div>
          </Card>
        </div>
...
export default function BHIVDashboardKit() {
  ...
  var dashState = useState("shakti"); var activeDash = dashState[0]; var setActiveDash = dashState[1];
